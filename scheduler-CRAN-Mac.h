...
//  uint64_t GetImsiFromRnti(uint16_t rnti);
  void SetInitialState(FfMacSchedSapUser* s);
  FfMacSchedSapUser* GetInitialState();
  void UpdateTraces(Ptr<LteUeNetDevice> ue, Ptr<LteEnbRrc> enbRrc, uint16_t rnti, uint8_t cellId);
  /////////////////////////////////
...
//
  ...
  */
  ...
  std::vector <Ptr<LteEnbRrc>> m_lista;

  std::map <uint16_t, schedMetricsRij> m_metricasUEs; // Mapa que contiene las metricas (por subbanda) para cada usuario

  FfMacSchedSapUser* m_schedInitialState;
  ...
  // Mapa de 3D con los valores de CQIs
  typedef std::map <uint64_t, std::vector <uint16_t>> imsiSubbands;
  std::map<uint32_t, imsiSubbands> m_matriz3Dcqis;
